...
import { getMatchup } from "../graphql/queries";
import { updateMatchup } from "../graphql/mutations";
export default function MatchupUpdateForm(props) {
  const {
    id: idProp,
    matchup: matchupModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getMatchup.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMatchup
        : matchupModelProp;
      setMatchupRecord(record);
    };
    queryData();
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateMatchup.replaceAll("__typename", ""),
            variables: {
              input: {
                id: matchupRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "MatchupUpdateForm")}
      {...rest}
    >
      <TextField
        label="Team1 hc"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || matchupModelProp)}
